...
//bool isFileOpen() { return isMounted() && fd_sd_card;/*file.isOpen();*/ }
...
//(dir_t* p,int printout,int index char* mach_name,)
int CardReader::countItems(SdFile dir) {
  dir_t p;
  return Linux_readDir(&p,0,-1,NULL);
  int c = 0;
  printf("countItems==\n");
...
void CardReader::selectByIndex(SdFile dir, const uint8_t index) {
  dir_t p;
	...
	return;
  for (uint8_t cnt = 0; Linux_readDir(&p,0,index,NULL) > 0;) {
...
void CardReader::selectByName(SdFile dir, const char * const match) {
  dir_t p;
  printf("selectByName==\n");
  Linux_readDir(&p,0,-1,match);
  return;
  for (uint8_t cnt = 0; Linux_readDir(&p,0,-1,match) > 0; cnt++) {
    if (is_dir_or_gcode(p)) {
      createFilename(filename, p);
...
void CardReader::startFileprint() {
	printf("startFileprint0==\n");
  if (isMounted()) {
  	printf("startFileprint1==\n");
...
void CardReader::openFile(char * const path, const bool read, const bool subcall/*=false*/) {
  ...
  //  if (file.open(curDir, fname, O_READ))
	if((fd_sd_card=fopen(pa_data,"r"))!=NULL)  
	{
...
void CardReader::checkautostart() {
  ...
  if (isMounted()
	...
	/*
    while (Linux_readDir(&p, nullptr,0) > 0) {
...
const char* CardReader::diveToFile(SdFile*& curDir, const char * const path, const bool echo/*=false*/) {
  ...
  while (item_name_adr) {
    // Find next sub
    char * const name_end = "";// luojin  strchr(item_name_adr, '/');
