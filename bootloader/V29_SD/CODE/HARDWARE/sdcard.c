...
SD_Error SD_ReadBlock(uint32_t addr, uint32_t *readbuff, uint16_t BlockSize)
{
  ...
  else if (DeviceMode == SD_DMA_MODE)
  {
	...
	count = 0xFFFFFF;		  											//by coolweedman
    while (DMA_GetFlagStatus(DMA2_FLAG_TC4) == RESET && count > 0)		//by coolweedman
    {
		count--;														//by coolweedman
	}
	if(count == 0)														//by coolweedman
	{
		return SD_ERROR; 												//by coolweedman
	}
  }
  return(errorstatus);
}
