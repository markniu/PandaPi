...
class Temperature {
  ...
  public:
	...
	static void setTargetFan(int16_t f_speed, const uint8_t e)
	{
		//int16_t fanSpeeds_old[FAN_COUNT];
		char tmp_data[32],cmd_buf[64];
		int cn=0;
		sprintf(tmp_data,"F%d,%d;",e,f_speed);
		printf(tmp_data);printf("\n");
		for(int i=0;i<strlen(tmp_data);i++)
			wiringPiI2CWriteReg8(i2c_fd, 8, tmp_data[i]);
		//wiringPiI2CWriteReg8(i2c_fd, 8, ';');
     ...
     */
       ...
       */
		...
		///////////////////
		 //////////I
		 sprintf(tmp_data,"I%.5f;",scalePID_i(PID_PARAM(Ki, e))/1000.0);
		 printf(tmp_data);printf("\n");
		 for(int i=0;i<strlen(tmp_data);i++)
		   wiringPiI2CWriteReg8(i2c_fd, 8, tmp_data[i]);
		  kk=millis();
		...
		///////////////////	D
		 ...
		 sprintf(tmp_data,"D%.5f;",scalePID_d(PID_PARAM(Kd, e)));
		 printf(tmp_data);printf("\n");
		 for(int i=0;i<strlen(tmp_data);i++)
		   wiringPiI2CWriteReg8(i2c_fd, 8, tmp_data[i]);
		  kk=millis();
