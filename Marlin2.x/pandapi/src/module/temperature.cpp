...
// public:
  ...
  void Temperature::set_fan_speed(uint8_t fan, uint16_t speed) {
    ...
    if (fan >= (FAN_COUNT+2)) return;// PANDAPI
	...
	int cn=0;
	sprintf(tmp_data,"F%d,%d;",fan,speed);
...
///////////////  PANDAPI
  ...
  #if ENABLED(EMERGENCY_PARSER)
    if (emergency_parser.killed_by_M112){  printf("killed_by_M112\n");kill(M112_KILL_STR, nullptr, true);}
  ...
  #endif
// 
  //PANDAPI if (!raw_temps_ready) return;

// PANDAPI  updateTemperaturesFromRawValues(); // also resets the watchdog
  ...
  millis_t ms = millis();
	...
	//  temp_range[e].maxtemp=310;
       // if (degHotend(e) > (temp_range[e].maxtemp )) {  printf("degHotend(e)==%f,%d,%d\n",degHotend(e),temp_range[e].maxtemp,e);max_temp_error((heater_id_t)e);}
      #endif
    ...
    filwidth.update_volumetric();
      ...
      if (degBed() > BED_MAXTEMP+200){printf("egBed() > BED_MAXTEMP %d,%f\n",H_BED,degBed()); max_temp_error(H_BED);}
    #endif
    ...
    do {
      ...
      if (degChamber() > CHAMBER_MAXTEMP){printf("H_CHAMBER==%d\n",H_CHAMBER); max_temp_error(H_CHAMBER);}
    #endif
      ...
      if (degCooler() > COOLER_MAXTEMP){printf("H_COOLER===%d\n",H_COOLER); max_temp_error(H_COOLER);}
    #endif
  ...
  TERN_(HAS_JOY_ADC_Z, joystick.z.reset());
    ...
    LOOP_L_N(e, COUNT(temp_dir)) {
      ...
      if (tdir) {
        ...
        if (rawtemp > temp_range[e].raw_max * tdir){printf("temp_range[e].raw_max * tdir,%d\n",temp_range[e].raw_max * tdir); max_temp_error((heater_id_t)e);}
    ...
    if (BEDCMP(temp_bed.raw, maxtemp_raw_BED)){ printf("H_BEDBERmax_temp_error==%d\n",H_BED); max_temp_error(H_BED);}
    ...
    if (CHAMBERCMP(temp_chamber.raw, maxtemp_raw_CHAMBER)){printf("H_CHAMBERmax_temp_error==%d\n",H_CHAMBER); max_temp_error(H_CHAMBER);}
    ...
    if (cutter.unitPower > 0) {
      if (COOLERCMP(temp_cooler.raw, maxtemp_raw_COOLER)){printf("H_COOLER max_temp_error==%d\n",H_COOLER); max_temp_error(H_COOLER);}
    }
...
{
	if(!strrchr(command, 'T'))
	{
		static int tt=0;
		if(tt==0)
		{
			tt=1;
			printf("no data from mcu\n");
		}
		return 1;
	}
	...
	if (apos) {
	  ...
	  if (strtol(apos + 1, NULL, 10) != checksum) {
		...
		//printf("i2c checksum mis:%d/%d,it's normal,\nraw data: %s ",checksum,strtol(apos + 1, NULL, 10),command);
		return 1;
	  }
	}
