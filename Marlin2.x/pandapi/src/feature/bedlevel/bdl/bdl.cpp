...
//M102   S4      Set the adjustable Z height value,e.g. M102 S4  means it 
will do adjusting while the Z height <=0.4mm , disable it by M102 S0.
...
void Bed_Distance_sensor_level::echo_name() { SERIAL_ECHOPGM("unified bed 
leveling"); }

void Bed_Distance_sensor_level::init(unsigned char _sda,unsigned char _scl,
int delay_s) {
...
void Bed_Distance_sensor_level::BD_sensor_process(void){
 ...
 if((millis()-timeout_auto)>timeout_y){
    ...
    if(BD_I2C_SENSOR.BD_Check_OddEven(tmp)&&(tmp&0x3ff)<1020){
      ...
      if(cur_z<0)
        BDsensor_config=0;
      //float abs_z=current_position.z>cur_z?(current_position.z-cur_z):(cur_z
-current_position.z);
      if(cur_z<(BDsensor_config/10.0)&&(BDsensor_config>0)
        &&(old_cur_z==cur_z)&&(old_buf_z==current_position.z)&&(z_sensor<(
MAX_BD_HEIGHT))){
    ...
    if(BDsensor_config==-5){
      ...
      for(int i=0;i<MAX_BD_HEIGHT*10;i++){
        tmp=BD_I2C_SENSOR.BD_i2c_read();    
        SERIAL_ECHOLNPGM("Calibrate data:",i,",",tmp&0x3ff,", check:",
BD_I2C_SENSOR.BD_Check_OddEven(tmp));
        safe_delay(500);
      }
      BDsensor_config=0; 
      ...
      if(BDsensor_config==-6){
       ...
       // delay(1000);
        ...
        if(z_pose>=MAX_BD_HEIGHT){
          ...
          float tmp_k=0;
          sprintf_P(tmp_1,  PSTR("G1 Z%d.%d"), (int)z_pose,(int)((int)(z_pose*
10)%10));
          parser.parse(tmp_1);
          gcode.process_parsed_command();
        //  current_position.z
