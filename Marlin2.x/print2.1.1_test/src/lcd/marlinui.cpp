...
void MarlinUI::init() {
  ...
  void MarlinUI::update() {
    ...
    void MarlinUI::update_buttons() {
      ...
      if (ELAPSED(now, next_button_update_ms)) {
        ...
        uint8_t enc = 0;
      if (buttons & EN_A) enc |= 0B01;//  PANDAPI
      if (buttons & EN_B) enc |= 0B10;
   ...
   */
  void MarlinUI::set_status(FSTR_P const fstr, int level) {
