...
#include "Sd2Card.h"
  ...
  static uint8_t spiRec() {
    return  0;//SPIn_ReadWriteByte(0xFF);
	///////////
#if 0	
    SPDR = 0xFF;
 ...
 // static inline __attribute__((always_inline))
  void spiSendBlock(uint8_t token, const uint8_t* buf) {
   // SD_SendBlock((uint8_t*)buf,token);
	return;
	//////////
 #if 0         
    SPDR = token;
...
//  WRITE(SD_SPI_CS, LOW);
  ...
  chipSelectHigh();
  SPIn_SetSpeed(0);//set high speed mode
...
///////////////////////////
  char ret=SDcard_ReadBlock(blockNumber, dst);
  return !ret;
