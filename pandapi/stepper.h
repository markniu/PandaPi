...
#if PANDAPI
	uint16_t intRes = (charIn1 * intIn2 >> 16);
#else
      ...
      *loops = multistep;
      ...
      { // higher step rate
        ...
        const uint32_t table_address = (uint32_t)&speed_lookuptable_fast[(uint8_t)(step_rate >> 8)][0],
          gain = (uint32_t)pgm_read_word_near((uint32_t*)(table_address + 2));
        timer = MultiU16X8toH16(tmp_step_rate, gain);
        timer = (uint16_t)pgm_read_word_near((uint16_t *)table_address) - timer;
      }
      else { // lower step rates
    	  uint32_t table_address = (uint32_t)&speed_lookuptable_slow[0][0];
        table_address += ((step_rate) >> 1) & 0xFFFC;
        timer = (uint16_t)pgm_read_word_near((uint16_t*)table_address)
              - (((uint16_t)pgm_read_word_near((uint16_t*)(table_address + 2)) * (uint8_t)(step_rate & 0x0007)) >> 3);
      }
