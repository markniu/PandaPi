 ...
 */
	//GPIO_ReadInputDataBit(Get_Port(IO),Get_Pin(IO))//

#define _READ(IO) digitalRead(IO)//READ_IO(IO)//((bool)(DIO ## IO ## _RPORT & _BV(DIO ## IO ## _PIN)))
...
#define _WRITE_C(IO, v)  /* do { if (v) { \
                                     ...
                                     while (0)
*/
#define _WRITE(IO, v)   digitalWrite(IO, v)//do{ printf("IO(%d:%d)",IO,v);  digitalWrite(IO, v); }while(0)// digitalWrite(IO, v)//WRITE_IO(IO, v) //do { if (&(DIO ## IO ## _RPORT) >= (uint8_t *)0x100) {_WRITE_C(IO, v); } else {_WRITE_NC(IO, v); }; } while (0)

#define _TOGGLE(IO)// do {DIO ## IO ## _RPORT ^= _BV(DIO ## IO ## _PIN); } while (0)

#define _SET_INPUT(IO)  pinMode(IO, INPUT) //GPIO_Set_INPUT(IO)// do {DIO ## IO ## _DDR &= ~_BV(DIO ## IO ## _PIN); } while (0)
#define _SET_OUTPUT(IO) pinMode(IO, OUTPUT) // GPIO_Set_OUTPUT(IO)// do {DIO ## IO ## _DDR |= _BV(DIO ## IO ## _PIN); } while (0)
...
#define SET_INPUT_PULLUP(IO)  do{ _SET_INPUT(IO);pullUpDnControl(IO,PUD_UP);/* _WRITE(IO, HIGH);*/ }while(0)
