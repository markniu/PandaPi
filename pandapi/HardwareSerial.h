...
public:
   ...
   // int available(void) override;
    ...
    int peek(void)
    {
        ...
        return 1;//uart_peek_char(_uart);
    }
    int read(void)
    {
        // return -1 when data is unvailable (arduino api)
        return 1;//uart_read_char(_uart);
    }
    // ::read(buffer, size): same as readBytes without timeout
    size_t read(char* buffer, size_t size)
    {
        return 1;//uart_read(_uart, buffer, size);
    }
    ...
    {
    	return 1;//return static_cast<int>(uart_tx_free(_uart));
    }
    void flush(void)  ;
    size_t write(uint8_t c)
    {
    	return 1;//return uart_write_char(_uart, c);
    }
    ...
    {
    	return 1;//return write((uint8_t) n);
    }
    ...
    {
    	return 1;// return uart_write(_uart, (const char*)buffer, size);
    }
    size_t write(const char *buffer)
    {
    	return 1;// return buffer? uart_write(_uart, buffer, strlen(buffer)): 0;
    }
    operator bool() const
    {
    	return 1;//return _uart != 0;
    }
    void setDebugOutput(bool);
    bool isTxEnabled(void)
    {
    	return 1;// return uart_tx_enabled(_uart);
    }
    bool isRxEnabled(void)
    {
    	return 1;// return uart_rx_enabled(_uart);
    }
    int baudRate(void)
    {
    	return 1;// return uart_get_baudrate(_uart);
    }

    bool hasOverrun(void)
    {
    	return 1;// return uart_has_overrun(_uart);
    }

    bool hasRxError(void)
    {
    	return 1;//return uart_has_rx_error(_uart);
    }
