...
test(int argc, char *argv[])
{
	...
	int cn=0;
	   ...
	   int data=0x31;
	  // wiringPiI2CWriteReg8(fd, 8, data);
	   wiringPiI2CWriteReg8 (i2c_fe, 0x0a, 8) ;
	...
	while(1)
	{
		...
		while( (data = getchar()) != '\n')
		{                        
			wiringPiI2CWriteReg8(i2c_fe, 8, data);
        }
		//data = getchar();
		
		while((data=wiringPiI2CReadReg8(i2c_fe,8))!='\0')
		{
			//if(data>0)
				printf("%c",data);
		}
